   ...
   function ajax_col()
   {
    ...
    function import_taobao()
    {
        ...
        else
        {
            ...
            #---这里是针对runmall改进的
            $origin_data = $this->import_from_csv($file['tmp_name'], false, 'gbk', CHARSET);
            ...
            $tmp_description = ROOT_PATH.'/data/tmp_description/';
            $tmp_image = ROOT_PATH.'/data/tmp_image/';

            $title = array_shift($origin_data);
            foreach ( $title as $key)
            {
                if( !array_key_exists($key,$my_map) )
                {
                    $this->_error('csv_fields_error');
                }//End Of If
            }//End Of Foreach
            if( count($title) != count($my_map) )
            {
            ...
            foreach($origin_data  as $row)
            {
                ...
                {
                    $row[$my_map[$title[$i]]] = $row[$i];
                    unset($row[$i]);
                }//End Of For
                //取出description
                if( file_exists($tmp_description.$row['sku'].'.docx' )){
                    $description = $this->read_file_docx($tmp_description.$row['sku'].'.docx');
                }//End Of Else If
            ...
            foreach ($records as $record)
            {
                ...
                //处理供应商逻辑
                $tmp_provider = explode(',',$record['provider']);
                $tmp_provider_sn = explode(',',$record['provider_sn']);
                $mys_tmp = array();
                if( count($tmp_provider) == count($tmp_provider_sn ) ) {
                    foreach($tmp_provider as $key => $one_provider) {
                        $mys_tmp[] = array($one_provider,$tmp_provider_sn[$key]);
                    }//End OF Foreach
                }//End Of IF
                ...
                $goods = array(
                    ...
                    //'default_image'      => $record['goods_image'][0]['thumbnail'],
                    'closed'             => 0,
                    'tags'               => $record['tags'],
                    'origin'             => $record['origin'],
                    'provider'           => json_encode($mys_tmp),
                    'brand'              => $record['brand'],
                );
                ...
                $row = &$record;
                $row['image_count'] = 0;
                if( file_exists($tmp_image.$row['sku'].'.jpg')) {
                    list($tmp_img['image_url'],$tmp_img['thumbnail'],$tmp_img['file_id']) = $this->_get_goods_image($tmp_image.$row['sku'].'.jpg',$goods_id);
                    $row['image_count']++;
                    $row['goods_image'][] = $tmp_img;
                } elseif( file_exists($tmp_image.$row['sku'].'.png')) {
                    list($tmp_img['image_url'],$tmp_img['thumbnail'],$tmp_img['file_id']) = $this->_get_goods_image($tmp_image.$row['sku'].'.png',$goods_id);
                    $row['image_count']++;
                    $row['goods_image'][] = $tmp_img;
                } elseif( file_exists($tmp_image.$row['sku'].'.gif')) {
                    list($tmp_img['image_url'],$tmp_img['thumbnail'],$tmp_img['file_id']) = $this->_get_goods_image($tmp_image.$row['sku'].'.gif',$goods_id);
                    $row['image_count']++;
                    $row['goods_image'][] = $tmp_img;
                }//End Of Func
                for($i=1; $i<10 ; $i++) //最多10张图片
                {
                    $tmp_img = array();
                    if( file_exists($tmp_image.$row['sku'].'-'.$i.'.jpg')) {
                        list($tmp_img['image_url'],$tmp_img['thumbnail'],$tmp_img['file_id']) = $this->_get_goods_image($tmp_image.$row['sku'].'-'.$i.'.jpg',$goods_id);
                        $row['image_count']++;
                        $row['goods_image'][] = $tmp_img;
                    } elseif( file_exists($tmp_image.$row['sku'].'-'.$i.'.png')) {
                        list($tmp_img['image_url'],$tmp_img['thumbnail'],$tmp_img['file_id']) = $this->_get_goods_image($tmp_image.$row['sku'].'-'.$i.'.png',$goods_id);
                        $row['image_count']++;
                        $row['goods_image'][] = $tmp_img;
                    } elseif( file_exists($tmp_image.$row['sku'].'-'.$i.'.gif')) {
                        list($tmp_img['image_url'],$tmp_img['thumbnail'],$tmp_img['file_id']) = $this->_get_goods_image($tmp_image.$row['sku'].'-'.$i.'.gif',$goods_id);
                        $row['image_count']++;
                        $row['goods_image'][] = $tmp_img;
                    }//End Of If
                }//End Of For
                ...
                /* 处理商品图片入库 */
                if( ! empty($record['goods_image']) ) {
                     foreach( $record['goods_image'] as $img_row ) {
                    /* 更新商品相册 */
                    $data = array(
                        'goods_id'      => $goods_id,
                        'image_url'  => $img_row['image_url'],
                        'thumbnail'  => $img_row['thumbnail'],
                        'sort_order' => 255,
                        'file_id'       => $img_row['file_id'],
                    );
                    $mod_goods_image = &m('goodsimage');
                    if (!$mod_goods_image->add($data))
                    {
                     ...
                     //设置默认图片,
                     $goods['default_image'] = $record['goods_image'][0]['thumbnail'];
                     $this->_goods_mod->edit($goods_id, $goods);
                }//End Of If
            ...
            }//End Of Insert One Data
            $this->show_message(sprintf(Lang::get('import_ok'), $num_record),
                'back_list', 'index.php?app=my_goods');
            return null;

            #----以下是原来的算法,不用了
            $origin = file_get_contents($file['tmp_name']);
            $csv_string = $this->unicodeToUtf8($origin);
     ...
     * @author maxmys
     */
    function _get_goods_image($filename,$my_goods_id) {
        ...
        $newdir = 'data/files/store_' . $this->visitor->get('manage_store') . '/goods_' . (time() % 200).'/';
        $newfile = date("YmdHis", time()) . $random.'.jpg';
        //create dir
        if (!ecm_mkdir(ROOT_PATH . '/' . $newdir))
        {
        ...
        /* 生成缩略图 */
        $thumbnail = dirname($new) . '/small_' . basename($new);
        make_thumb($new,$thumbnail, THUMB_WIDTH, THUMB_HEIGHT, THUMB_QUALITY);
        ...
        return array($new,$thumbnail,$file_id);
    }//End Of Func
     ...
     * @author maxmys
     */
    function EffectiveInsertGoods ( $records ) {
