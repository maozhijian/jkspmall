...
class Captcha
{
    ...
    function ttffont()
    {
        ...
        for ($i = 0; $i < $seccodelength; $i++)
        {
            ...
            if($font[$i][$this->height] == null)
            {
               $font[$i][$this->height] = 0;
            }
            if( $font[0]['angle'] > 0)
            {
                if($font[$i['height']] < $this->height)
                {
                    $y = mt_rand($font[$i]['height'], $this->height);
                }
                else
                {
                    $y = mt_rand($this->height,$font[$i]['height']);
                }
            }
            else
            {
                $y = mt_rand($font[$i]['height'] - $font[$i]['hd'], $this->height - $font[$i]['hd']);
                if($font[$i]['height'] - $font[$i]['hd'] < $this->height - $font[$i]['hd'])
                {
                    $y = mt_rand($font[$i]['height'] - $font[$i]['hd'], $this->height - $font[$i]['hd']);
                }
                else
                {
                    $y = mt_rand( $this->height - $font[$i]['hd'], $font[$i]['height'] - $font[$i]['hd']);
                }
            }
